			...
			trackerChannel.retrieveMessageById(926572972578328658L).queue((message) -> {
				...
				if (content.length() < 1990) {
					...
					if (content.contains(authorTag.substring(0, authorTag.length() - 5) + " : ")) {
						...
						event.getChannel().sendMessage(content.substring(content.indexOf(authorTag.substring(0, authorTag.length() - 5)), authorTag.substring(0, authorTag.length() - 2).length() + counter.length())).queue();
